...
function scene:create( event )
	...
	background = display.newImageRect( sceneGroup, "ui/background/gameover.png", 700, 400 )
	...
	local highScoresHeader = display.newText( sceneGroup, "High Scores", display.contentCenterX +5, 50, native.systemFont, 30 )
    highScoresHeader:setFillColor( 0.1 )
	for i = 1, 5 do
		if ( scoresTable[i] ) then
			local yPos = 130 + ( i * 23 )

			local rankNum = display.newText( sceneGroup, i .. ")", display.contentCenterX - 10 , yPos -70, native.systemFont, 15 )
			rankNum:setFillColor( 0.1 )
			rankNum.anchorX = 1

			local thisScore = display.newText( sceneGroup, scoresTable[i], display.contentCenterX , yPos -70, native.systemFont, 25 )
            thisScore.anchorX = 0
