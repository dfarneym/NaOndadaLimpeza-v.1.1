...
function level1:createInvoices(currentLevel)
	...
	invoices[invCount] = display.newImageRect(base.bills[numBills].path, 80, 60)
	...
	for a = 0, invCount, 1 do
		if invoices[a] ~= nil and invoices[a].x ~= nil then
			if invoices[a].x < -100 then
				...
				invoices[a].x = invoices[a].x  - (14/2)  -- spped/2
			end
		end
	end
end
...
function level1:createIncomes(currentLevel)
	...
	incomes[incCount] = display.newImageRect(base.moneys[numColl].path, 55, 50)
